...
export default function DailyTarot() {
  ...
  // စစချင်းမှာ ကဒ်တစ်ခုကို ကြိုတင်ရွေးထားမယ် (ဒါမှမဟုတ် Random ယူထားမယ်)
  useEffect(() => {
    drawNewCard(false);
  }, []);
  ...
  const drawNewCard = (animate = true) => {
    ...
    if (animate) {
      setIsFlipped(false);
      setIsAnimating(true);
      // ကဒ်ပြန်လှန်ပြီးမှ အသစ်ပြောင်းမယ်
      setTimeout(() => {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900 via-purple-900 to-slate-900 flex flex-col items-center justify-center p-4 text-white">
      ...
      {/* Meaning Section (Shows only when flipped) */}
      ...
      {/* CSS Utility for 3D Transform (In case Tailwind config misses plugins) */}
      <style jsx global>{`
